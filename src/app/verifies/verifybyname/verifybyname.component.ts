...
@Component({
  selector: 'app-verifybyname',
  standalone: true,
  templateUrl: './verifybyname.component.html',
  styleUrls: ['./verifybyname.component.scss'],
...
export class VerifybynameComponent implements OnInit {
  ...
  initDataSource() {
    ...
    if (!this.filterValue) {
      ...
      this.findByNameAndSurname(filter)
    }
  ...
  findByNameAndSurname(firstAndLastName: string) {
    let firstAndLastNames: string[] = [];
    // if (firstAndLastName.includes(' ')) {
    firstAndLastNames = firstAndLastName.split(' ');
    this.loading = !this.loading;
    let page: number = this.pageIndex;
    let size: number = this.pageSize;
    this.paginator.pageIndex = 0;
    this.api.getAllVerifyByNameResultsWithPageFilter(page, size, firstAndLastNames[0], firstAndLastNames[1]).pipe(
      map((verifyResultsData: any) => this.dataSource = verifyResultsData.response)
    ).subscribe()
    this.filterValue = firstAndLastName;
    this.loading = !this.loading;
