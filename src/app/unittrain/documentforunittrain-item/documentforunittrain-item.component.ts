...
import { FormGroup, FormBuilder, Validators, ControlContainer, NgModel, NgForm, MaxLengthValidator, FormsModule, ReactiveFormsModule, FormControlName, FormControl } from '@angular/forms';
...
import { ThdatePipe } from 'src/app/pipes/thdate.pipe';
import { ThYearPipe } from "../../pipes/th-year.pipe";
import { DataService } from 'src/app/services/data.service';
import { departmentlistComponent } from "../../list/departmentlist/departmentlist.component";

@Component({
  selector: 'app-documentforunittrain-item',
  standalone: true,
  templateUrl: './documentforunittrain-item.component.html',
  styleUrls: ['./documentforunittrain-item.component.scss'],
  imports: [CommonModule, MatIconModule, FormsModule, ReactiveFormsModule, MatFormFieldModule, FlexLayoutModule,
    ...
    MatInputModule, MatSelectModule, MatCheckboxModule, MatMenuModule, MatDatepickerModule, MatNativeDateModule, ProvinceslistComponent, UnittrainlistComponent, UnitlistComponent, ThYearPipe, departmentlistComponent]
})

export class DocumentItemComponent {
  documentForm !: FormGroup;
  actionBtn: string = "บันทึก";
  title: string = "รายการเลขที่หนังสือที่ขอนำตัวขึ้นทะเบียนและนำปลดฯ";
  isDelete!: boolean;
  ...
  ngOnInit(): void {
    this.documentForm.controls['id'].setValue('');
    this.documentForm.controls['doc_NO'].setValue('');
    this.documentForm.controls['doc_NAME'].setValue('');
    this.documentForm.controls['doc_DATE'].setValue('');
    this.documentForm.controls['doc_DEPARTMENT_ID'].setValue('');
    this.documentForm.controls['recruit_UNIT_ID'].setValue('');
    this.documentForm.controls['recruit_UNIT_SHORTNAME'].setValue('');
    this.documentForm.controls['recruit_PROVINCE_ID'].setValue('');
    this.documentForm.controls['unit_TRAIN_ID'].setValue('');
    this.documentForm.controls['unit_TRAIN_SHORTNAME'].setValue('');
    this.documentForm.controls['at_YEAR'].setValue('');
    this.documentForm.controls['user_VERIFY'].setValue('');
    this.documentForm.controls['doc_NOTE'].setValue('');
    ...
    if (this.data.isDelete) {
      ...
      this.departmentSelectDisable = "ture";
      this.findDocumentById(this.data.docId);
  ...
  updateRequest() {
    ...
    this.api.putRequest(this.documentForm.value)
      .subscribe({
        next: (res) => {
