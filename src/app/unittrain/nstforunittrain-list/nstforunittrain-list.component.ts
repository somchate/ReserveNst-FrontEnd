...
@Component({
  selector: 'app-nstforunittrain-list',
  standalone: true,
  templateUrl: './nstforunittrain-list.component.html',
  styleUrls: ['./nstforunittrain-list.component.scss'],
  imports: [CommonModule, MatIconModule, FormsModule, ReactiveFormsModule, MatFormFieldModule,
    ...
    ProvinceslistComponent, UnittrainlistComponent, SchoolslistComponent, EduatyearlistComponent, departmentlistComponent]
})
export class NstforunittrainlistComponent implements OnInit {

  title = "ตรวจสอบ นศท.->หนังสือนำตัวขึ้นทะเบียน/นำปลดฯ";
  ...
  ngDoCheck(): void {
    this.documentFormGroup.controls['documentDept'].setValue(this.data.departmentId);
    this.documentFormGroup.controls['recruitProvince'].setValue(this.data.recruitProvince);
  }
  ...
  getSelectDepartmentChangeValue(selectChangeValue: any) {
    ...
    this.nstFormGroup.controls['documentDept'].setValue(selectChangeValue);
